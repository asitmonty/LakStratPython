...
def generateFortClusters(playerIDs, allianceIDs, max_fort_radius):
    habitat_column_names = [COLUMN_ID, COLUMN_MAPX, COLUMN_MAPY, COLUMN_PLAYERID]
    ...
    if playerIDs.values():
        df_player_habitat_data = tbl_habitat.read_from_sql_to_dataframe(1, playerIDs.values())  # load selected rows fromt the  habitat table to a dataframe
        playerIDList = playerIDs.values()
    if allianceIDs.values():
        df_player_habitat_data = tbl_habitat.read_from_sql_to_dataframe_alliance(1, allianceIDs.values())  # load selected rows fromt the habitat table to a dataframe
...
def main():
    ...
    alliance_distance_list = generateFortClusters(playerIDs, alliance_legends, max_fort_radius)  #one of playerIDs and allianceIDs have the value, other is null
